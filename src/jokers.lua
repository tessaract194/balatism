SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, card)
        local numerator, denominator = SMODS.get_probability_vars(card, 7, card.ability.extra.odds_mult, "sevennn_tracksMult")
        local numerator2, denominator2 = SMODS.get_probability_vars(card, 17, card.ability.extra.odds_money, "sevennn_tracksMoney")
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
            if context.other_card:get_id() == 7 and SMODS.pseudorandom_probability(card, "sevennn_tracksMult", 7, card.ability.extra.odds_mult) then
    ...
    calc_dollar_bonus = function(self, card)
        if SMODS.pseudorandom_probability(card, "sevennn_tracksMoney", 17, card.ability.extra.odds_money) then
